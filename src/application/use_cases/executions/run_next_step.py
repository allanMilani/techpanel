...
class RunNextStep:
    ...
    async def execute(self, dto: RunNextStepInputDTO) -> None:
        while True:
            ...
            next_step = await self.pipeline_repo.get_next_step(
                execution.pipeline_id,
                after_order,
            )
            ...
            if exit_code == 0:
                await self.step_execution_repo.update(
                    running.mark_success(log_output, exit_code)
                )
            else:
                await self.step_execution_repo.update(
                    running.mark_failed(log_output, exit_code)
                )
